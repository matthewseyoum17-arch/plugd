...
export function StatusBadge({ status }: { status: string }) {
  const style = statusStyles[status] || 'bg-white/5 text-gray-400 border border-white/10'
  return (
    <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider font-button ${style}`}>
      {status.replace('_', ' ')}
    </span>
  )
}
