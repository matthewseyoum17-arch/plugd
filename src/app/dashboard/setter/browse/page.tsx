      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {listings?.map((listing) => (
          <div key={listing.id} className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-6">
            ...
            <Link
              href={`/dashboard/setter/browse/${listing.id}`}
              className="block w-full py-2 text-center bg-[#00FF94] text-black font-medium rounded-lg hover:bg-[#00cc76] transition-colors"
            >
              Apply to Promote
            </Link>
          </div>
