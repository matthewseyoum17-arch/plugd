  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-400 mt-2 font-medium">Review and manage setters who want to promote your products.</p>
      </div>
      ...
      <div className="bg-glass-bg border border-glass-border backdrop-blur-md rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full">
          <thead className="bg-black/40 border-b border-white/5">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-button font-semibold text-gray-400 uppercase tracking-wider">Setter</th>
              <th className="px-6 py-4 text-left text-xs font-button font-semibold text-gray-400 uppercase tracking-wider">Product</th>
              <th className="px-6 py-4 text-left text-xs font-button font-semibold text-gray-400 uppercase tracking-wider">Applied</th>
            ...
            {applications?.map((app) => (
              <tr key={app.id} className="hover:bg-white/5 transition-colors group">
                <td className="px-6 py-4">
                  <div className="text-sm font-medium text-white">{app.setter_profiles?.users?.full_name || 'Unknown'}</div>
                  <div className="text-xs text-gray-500 mt-0.5">{app.setter_profiles?.users?.email}</div>
                </td>
                <td className="px-6 py-4 text-sm font-medium text-gray-300">{app.listings?.title || 'N/A'}</td>
