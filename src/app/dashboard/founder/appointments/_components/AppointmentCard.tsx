...
function CountdownTimer({ submittedAt }: { submittedAt: string }) {
  ...
  if (remaining <= 0)
    return (
      <span className="text-cyan-400 font-button font-medium text-xs tracking-wide uppercase">
        Auto-approved
      </span>
    );
...
export function AppointmentCard(props: Props) {
  ...
  const handleConfirm = () => {
    ...
    const commission =
      props.appointmentType === "appointment"
      ...
      <div className="flex items-start justify-between mb-5">
        <div className="flex-1 min-w-0 pr-4">
          ...
          <p className="text-sm font-medium text-gray-400 mt-1">
            by <span className="text-gray-300">{props.setterName}</span>
          </p>
        </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-glass-border">
        <div className="flex items-center gap-4">
          ...
          {optimisticStatus === "submitted" && (
            <CountdownTimer submittedAt={props.submittedAt} />
          )}
        </div>
